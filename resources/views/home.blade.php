    ...
    <section class="py-5 bg-white">
        <div class="container">
            ...
            <div class="row g-4" style="margin-bottom: 80px;">
                @foreach($vehicles->reverse() as $vehicle)
                    <div class="col-md-4">
                        <div class="card shadow-sm rounded-4 bg-light border-0 p-3 text-start h-100">
                            <img src="{{ asset('storage/' . $vehicle->gambar) }}" class="card-img-top mx-auto"
                                style="max-height: 150px; width: auto;" alt="{{ $vehicle->nama_kendaraan }}">
                            <div class="card-body">
                                <div class="mb-2 text-warning d-flex align-items-center gap-1">
                                    @for($i = 1; $i <= 5; $i++)
                                        @if($i <= round($vehicle->rating ?? 4)) {{-- Default 4 bintang jika kosong --}}
                                            <i class="bi bi-star-fill"></i>
                                        @else
                                            <i class="bi bi-star"></i>
                                        @endif
                                    @endfor
                                    <span class="text-muted ms-1">({{ $vehicle->reviews ?? 65 }})</span>
                                </div>
                                <h6 class="fw-bold">{{ $vehicle->nama_kendaraan }}</h6>
                                <hr class="my-2">
                                <div class="d-flex justify-content-between align-items-center">
                                    <div class="fw-bold text-dark fs-5">
                                        Rp{{ number_format($vehicle->harga, 0, ',', '.') }}
                                        <small class="text-muted fw-normal">/Hari</small>
                                    </div>
                                    <a href="{{ route('mobil.detail', ['id' => $vehicle->id]) }}"
                ...
                <div class="container">
                    ...
                    <div class="row g-4" style="margin-bottom: 80px;">
                        @foreach($motors as $motor)
                            <div class="col-md-4">
                                <div class="card shadow-sm rounded-4 bg-light border-0 p-3 text-start h-100">
                                    <img src="{{ asset('storage/' . $motor->gambar) }}" class="card-img-top mx-auto"
                                        style="max-height: 150px; width: auto;" alt="{{ $motor->nama_kendaraan }}">
                                    <div class="card-body">
                                        <div class="mb-2 text-warning d-flex align-items-center gap-1">
                                            @for($i = 1; $i <= 5; $i++)
                                                @if($i <= round($motor->rating ?? 4)) {{-- Default 4 bintang jika kosong --}}
                                                    <i class="bi bi-star-fill"></i>
                                                @else
                                                    <i class="bi bi-star"></i>
                                                @endif
                                            @endfor
                                            <span class="text-muted ms-1">({{ $motor->reviews ?? 65 }})</span>
                                        </div>
                                        <h6 class="fw-bold">{{ $motor->nama_kendaraan }}</h6>
                                        <hr class="my-2">
                                        <div class="d-flex justify-content-between align-items-center">
                                            <div class="fw-bold text-dark fs-5">
